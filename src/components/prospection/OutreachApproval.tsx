...
export function OutreachApproval({
  ...
  if (approved) {
    return (
      <div className="rounded-xl border border-[#FF6B3D]/15 bg-[#FF6B3D]/5 p-3">
        <div className="flex items-center justify-between gap-2">
          <span className="flex items-center gap-1.5 text-xs text-[#FF6B3D]">
          ...
          <button
            ...
            onClick={revoke}
            disabled={pending}
            className="text-[#969BA8] hover:text-white"
        ...
        {message && (
          <p className="mt-1 text-[11px] text-[#969BA8]">{message}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-3">
      <p className="mb-2 flex items-center gap-1.5 text-xs text-[#969BA8]">
      ...
      {actionableContacts.length > 0 && (
        <select
          ...
          className="mb-2 w-full rounded-lg border border-white/[0.08] bg-[#0B0D12] px-2.5 py-2 text-xs text-white"
        >
      ...
      <button
        ...
        className="rounded-full bg-[#FF6B3D] px-3 py-1.5 text-xs font-semibold text-[#0B0D12] disabled:opacity-40"
      >
